...
from main.util.file_util import get_file_name_from_path
...
path_with_end_slash = {EXTENSION.WITHOUT.value: 'home/data/src/file/', EXTENSION.WITH.value: 'home/data/src/file.txt/'}
path_with_beg_slash = {EXTENSION.WITHOUT.value: '/home/data/src/file', EXTENSION.WITH.value: '/home/data/src/file.txt'}
...
get_file_name_from_path_data = {PATHS_TEST_DATA.PATHS.value: [path_with_end_slash, path_with_beg_slash, path_with_both_slashes, path_without_slash],
...
file_name_with_end_slash = {EXTENSION.WITHOUT.value: 'file/', EXTENSION.WITH.value: 'file.txt/'}
file_name_with_beg_slash = {EXTENSION.WITHOUT.value: '/file', EXTENSION.WITH.value: '/file.txt'}
...
get_file_name_from_file_name_data = {PATHS_TEST_DATA.PATHS.value: [file_name_with_end_slash, file_name_with_beg_slash, file_name_with_both_slashes, file_name_without_slash],
...
def test_paths_data(self, paths_data_dict: dict):
    for path in paths_data_dict[PATHS_TEST_DATA.PATHS.value]:
        ...
        with self.assertRaises(ValueError):
            get_file_name_from_path(path[EXTENSION.WITHOUT.value], True)

        # if passed path has no extensions, and we want to return a filename without extension:
        file_name = get_file_name_from_path(path[EXTENSION.WITHOUT.value], False)
        ...
        file_name = get_file_name_from_path(path[EXTENSION.WITH.value], False)
        self.assertTrue(file_name == paths_data_dict[PATHS_TEST_DATA.FILE_NAME.value][EXTENSION.WITHOUT.value])

        # if passed path has an extension, and we want to return a filename with the extension:
        file_name = get_file_name_from_path(path[EXTENSION.WITH.value], True)
