...
def get_expected_out(solutions: pd.DataFrame, start_index: int, end_index: int) -> Tuple[int, List[AtiItem], ast.AST]:
    ...
    tree = get_canonicalized_form(fragment)
    for i in range(start_index, end_index):
        ati_elements.append(__get_ati_data(solutions, i))
    return end_index, ati_elements, tree
...
class TestFindSomeFragments(LoggedTest):
    ...
    def test_first_index(self) -> None:
        ...
        actual_out = get_actual_out(solutions, 0)
        expected_out = get_expected_out(solutions, 0, COUNT_SOURCE_1)
        self.assertTrue(are_equal(expected_out, actual_out))

    def test_middle_index(self) -> None:
        solutions = create_solutions()
        actual_out = get_actual_out(solutions, 3)
        expected_out = get_expected_out(solutions, 3, COUNT_SOURCE_1)
        self.assertTrue(are_equal(expected_out, actual_out))

    def test_middle_index_with_other_fragment(self) -> None:
        solutions = create_solutions()
        actual_out = get_actual_out(solutions, COUNT_SOURCE_1)
        expected_out = get_expected_out(solutions, COUNT_SOURCE_1, COUNT_SOURCE_1 + COUNT_SOURCE_2)
        self.assertTrue(are_equal(expected_out, actual_out))

    def test_last_index(self) -> None:
        solutions = create_solutions()
        actual_out = get_actual_out(solutions, COUNT_SOURCE_1 + COUNT_SOURCE_2 - 1)
        expected_out = get_expected_out(solutions, COUNT_SOURCE_1 + COUNT_SOURCE_2 - 1, COUNT_SOURCE_1 + COUNT_SOURCE_2)
        self.assertTrue(are_equal(expected_out, actual_out))
