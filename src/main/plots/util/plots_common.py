...
from src.main.util.consts import EXTENSION, DEFAULT_VALUE, INVALID_FILE_FOR_PREPROCESSING, TASK_TRACKER_COLUMN
...
def fill_seconds_columns(data: pd.DataFrame) -> None:
    first_datetime = pd.to_datetime(data[TASK_TRACKER_COLUMN.DATE.value].iloc[0])
    data[CT_SECONDS_COL] = pd.to_datetime(data[TASK_TRACKER_COLUMN.DATE.value], errors='ignore')
