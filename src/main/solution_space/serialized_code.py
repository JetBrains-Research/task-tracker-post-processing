...
class SerializedCode(IdCounter, PrettyString):
    ...
    def rewrite_code_info(self, new_folder_with_files: str) -> None:
    ...
    def __create_files_for_trees(self, to_rewrite: bool = False) -> None:
        self.__create_file_for_tree(self._canon_tree, TREE_TYPE.CANON.value, to_rewrite=to_rewrite)
        for i, anon_tree in enumerate(self._anon_trees):
            self.__create_file_for_tree(anon_tree, f'{TREE_TYPE.ANON.value}_{i}', to_rewrite=to_rewrite)
    ...
    def __create_file_for_tree(self, tree: ast.AST, str_tree_type: str,
                               to_rewrite: bool = False) -> str:
        if self._file_by_tree_dict.get(tree) is not None and not to_rewrite:
