...
from src.main.solution_space.consts import DIFFS_PERCENT_TO_GO_DIRECTLY, DISTANCE_TO_GRAPH_THRESHOLD, \
    ROLLBACK_PROBABILITY, NODE_NUMBERS_RATE, TOP_N_CANON
...
class PathFinderV3(IPathFinder):
    ...
    def __find_closest_vertex(self, user_vertex: Vertex, goal: Vertex) -> Optional[Vertex]:
        ...
        # We want to have TOP_N_CANON trees with nodes number, that is close to user_nodes_number.
        # So we consequently add vertices with node numbers equal:
        # 1. user_nodes_number
        # 2. user_nodes_number - 1, user_nodes_number + 1
        # 3. user_nodes_number - 2, user_nodes_number + 2
        # 4. ....
        # until we reach TOP_N_CANON or have no more node_numbers to add

        user_canon_nodes_number = get_nodes_number_in_ast(user_vertex.canon_tree)
        candidates = []
        nodes_numbers = collections.deque([user_canon_nodes_number])
        indent = 0
        while len(candidates) < TOP_N_CANON and nodes_numbers:
            while nodes_numbers:
                nodes_number = nodes_numbers.pop()
                candidates += [Vertex.get_item_by_id(id) for id in self._graph.canon_trees_nodes_number[nodes_number]]
            indent += 1
            lower_bound = user_canon_nodes_number - indent
            upper_bound = user_canon_nodes_number + indent
            if lower_bound >= 0:
                nodes_numbers.append(lower_bound)
            if upper_bound <= max(self._graph.canon_trees_nodes_number.keys()):
                nodes_numbers.append(upper_bound)

        # Todo: also choose anon_trees?
