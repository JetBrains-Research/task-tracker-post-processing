...
from src.main.solution_space.solution_space_handler import get_rate, get_task_index, get_enum_or_default
...
def sample_n_correct_test_inputs(pseudo_solutions_path: str,
    ...
    dataframes = list(map(lambda file: pd.read_csv(file, encoding=ISO_ENCODING), files))
    merged_df = pd.concat(dataframes, ignore_index=True)

    merged_df = merged_df[merged_df[CODE_TRACKER_COLUMN.AGE.value] != DEFAULT_VALUE.AGE.value and
                          merged_df[CODE_TRACKER_COLUMN.AGE.value] != INVALID_FILE_FOR_PREPROCESSING and
                          merged_df[CODE_TRACKER_COLUMN.EXPERIENCE.value] != DEFAULT_VALUE.INT_EXPERIENCE.value]
    ...
    if specific_indices:
        checked_indices = merged_df.index.isin(specific_indices)
        if all(checked_indices):
            n_random_rows = merged_df[checked_indices]
        else:
            log_and_raise_error('Passed specific indices are not in merged_df', log)
    else:
        try:
    ...
    for i, row in n_random_rows.iterrows():
        ...
        test_inputs.append({TEST_INPUT.SOURCE_CODE: row[CODE_TRACKER_COLUMN.FRAGMENT.value].rstrip('\n'),
                            ...
                            TEST_INPUT.INT_EXPERIENCE:
                                get_enum_or_default(INT_EXPERIENCE,
