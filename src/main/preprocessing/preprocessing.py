...
def __partition_into_ct_and_ati_files(files: List[str]) -> Tuple[List[str], List[str]]:
    ati_files = [f for f in files if 'ide-events' in f]
...
def preprocessing(path: str) -> str:
    ...
    for user_folder in user_folders:
        ...
        for task_folder in task_folders:
            task_path = os.path.join(user_path, task_folder)
            ...
            if __handle_ct_files(ct_files, output_task_path):
                if ati_files:
