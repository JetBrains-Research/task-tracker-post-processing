        ...
        if type(imp) == ast.Import:
            ...
            if hasattr(imp, "module"):
                if imp.module in supportedLibraries:
                    ...
                    log.info(f'astTools\timportedName\tUnsupported library: {printFunction(imp)}, bug')

            else:
                log.info(f'astTools\timportedName\tWhy no module? {printFunction(imp)}, bug')
    return False
                    ...
                    for pair in allIds:
                        if pair[0] == currentId:
                            if pair[1] == None:
                                ...
                                log.info(
                                    f'astTools\tgatherAllVariables\tConflicting originalIds? {pair[0]} : {pair[1]}, '
                                    f'{origName}\n{printFunction(a)}, bug')
                            break
                    else:
                        allIds |= {(currentId, origName)}
...
def gatherAssignedVars(targets):
    ...
    for target in targets:
        if type(target) in [ast.Tuple, ast.List]:
            ...
            log.info(f'astTools\tgatherAssignedVars\tWeird Assign Type: {str(type(target))}, bug')
    return newTargets
        ...
        if type(child) == ast.Import:
            for alias in child.names:
                if alias.name in supportedLibraries:
                    ...
                    log.info(f'astTools\tgetAllImports\tUnknown library: {alias.name}, bug')
            ...
            if child.module in supportedLibraries:
                for alias in child.names:  # these are all functions
                    if alias.name in libraryMap[child.module]:
                        ...
                        log.info(
                            f'astTools\tgetAllImports\tUnknown import from name: {child.module}, {alias.name}, bug')
            else:
                log.info(f'astTools\tgetAllImports\tUnknown library: {child.module}, bug')
    return imports
...
def num_negate(op):
    ...
    if top == ast.Add:
        ...
        log.info(f'astTools\tnum_negate\tUnusual type: {str(top)}, bug')
...
def eventualType(a):
    ...
    if not isinstance(a, ast.AST):
        ...
        if t == None:
            ...
            log.info(f'astTools\teventualType\tUnknown type in subscript: {str(t)}, bug')
        ...
        log.info(f'astTools\teventualType\tUnimplemented type {str(type(a))}, bug')
    return None
...
def deepcopy(a):
    ...
    if type(a) == list:
        ...
        return a
    if not isinstance(a, ast.AST):
        log.info(f'astTools\tdeepcopy\tNot an AST: {str(type(a))}, bug')
    ...
    if type(a) in [ast.And, ast.Or, ast.Add, ast.Sub, ast.Mult, ast.Div,
        ...
        log.info(f'astTools\tdeepcopy\tNot implemented: {str(type(a))}, bug')
    cp = copy.deepcopy(a)
...
def exportToJson(a):
    """Export the canonicalization to json format"""
    if a == None:
        return "null"
        ...
        log.info(f'astTools\texportToJson\tMissing type: {str(type(a))}, bug')
    ...
    if type(a) in astNames:
        ...
        log.info(f'astTools\texportToJson\tMissing AST type: {str(type(a))}, bug')
    s += "}"
    return s
...
def astFormat(x, gid=None):
    ...
    if type(x) in [int, float, complex]:
        ...
        log.info(f'astTools\tastFormat\t{str(type(x))}, {str(x)}, bug')
    return None
