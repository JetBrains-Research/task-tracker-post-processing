...
def updateVariableNames(a, varMap, scopeName, randomCounter, imports):
    ...
    if type(a) in [ast.FunctionDef, ast.ClassDef]:
        ...
        if a.arg not in varMap and not (builtInName(a.arg) or importedName(a.arg, imports)):
            log.info(f'Can not assign to arg?, bug')
        if a.arg in varMap:
            if not hasattr(a, "originalId"):
                a.originalId = a.arg
            if varMap[a.arg][0] == "r":
    ...
    if type(a) == ast.FunctionDef:
        for param in a.args.args:
            if type(param) == ast.arg:
                ...
                log.info(f'transformations\tanonymizeFunctionNames\tWeird parameter type: {str(type(param))}, bug')
    if goBackwards:
...
def findHelperFunction(a, helperId, helperCount):
    """Finds the first helper function used in the canonicalization"""
    if not isinstance(a, ast.AST):
        return None
        ...
        while countVariables(body[i], helper.name) > helperCount:
            ...
            for j in range(len(callExpression.args)):
                ...
                if countVariables(methodLines, arg.arg) == 1 and countVariables(methodLines[-1], arg.arg) == 1:
                    ...
                    if type(arg) == ast.arg:
                        ...
                        log.info(f'transformations\tmapHelper\tWeird arg: {str(type(arg))}, bug')
            if badArgs:
                helperCount += 1
                continue
            ...
            if countOccurances(helper, ast.Return) == 1:
                ...
                if type(body[i]) == ast.Return and comp:
                    ...
                    callVal = ast.NameConstant(None, helperReturnVal=True)
                    transferMetaData(callExpression, callVal)
                    assignVal = ast.Assign(body[i].targets, callVal, global_id=body[i].global_id,
                                           helperReturnAssign=True)
                    ...
                    returnLine = []
                    log.info(f'transformations\tmapHelper\tWeird removeCall: {str(type(body[i]))}, bug')
            log.info(f'transformations\tmapHelper\tMapped helper function: {helper.name}, bug')
        ...
        if len(a.targets) > 1:
            ...
            for i in range(len(a.targets) - 1, 0, -1):
                ...
                if type(t) == ast.Name:
                    ...
                    log.info(f'transformations\tsimplify\tOdd loadedTarget: {str(type(t))}, bug')
    ...
    if not isinstance(a, ast.AST):
        ...
        for i in range(len(a.args.args)):
            ...
            if type(arg) == ast.arg:
                ...
                log.info(f'transformations\tpropogateMetadata\tWeird type in args: {str(type(arg))}, bug')
            ...
            if type(t) == ast.Name:
                ...
                for j in range(len(t.elts)):
                    ...
                    if type(t2) == ast.Name:
                        ...
                        log.info(f'transformations\tpropogateMetadata\tOdd listTarget: {str(type(t.elts[j]))}, bug')
        return a
    elif type(a) == ast.AugAssign:
        # Turn all AugAssigns into Assigns
        ...
        for key in variableMap1:
            if key in variableMap2:
                if variableMap1[key] == variableMap2[key]:
                    ...
                    log.info(f'transformations\tsimplify\tvarId mismatch, bug')
                else:  # type mismatch
...
def constantFolding(a, ops_index=-1):
    ...
    if t in [ast.FunctionDef, ast.ClassDef]:
        ...
        if allConstant and (type(a.func) == ast.Name) and (a.func.id in builtInFunctions.keys()) and \
            ...
            try:
                ...
                log.info(f'transformations\tconstantFolding\tFunction crashed: {str(a.func.id)}, bug')
                pass
        for i in range(len(a.args)):
    ...
    # This needs to be separate because the attribute is a string
        ...
        return a
    else:  # All statements, canonicalization.Lambda, canonicalization.Dict, canonicalization.Set, canonicalization.Repr, canonicalization.Attribute, canonicalization.Subscript, etc.
...
def isMutatingFunction(a):
    ...
    if type(a) != ast.Call:  # Can only call this on Calls!
        log.info(f'transformations\tisMutatingFunction\tNot a Call: {str(type(a))}, bug')
        return True
...
def allVariableNamesUsed(a):
    """Gathers all the variable names used in the canonicalization"""
    if not isinstance(a, ast.AST):
        return []
    elif type(a) == ast.Name:
        return [a.id]
...
def hasMutatingFunction(a):
    """Checks to see if the canonicalization has any potentially mutating functions"""
    if not isinstance(a, ast.AST):
        return False
            ...
            for elt in a.target.elts:
                if type(elt) == ast.Name:
                    ...
                    if type(elt.value) == ast.Name:
                        ...
                        log.info(f'transformations\tclearBlockVars\tFor target subscript not a name: {str(type(elt.value))}, bug')
                else:
                    log.info(f'transformations\tclearBlockVars\tFor target not a name: {str(type(elt))}, bug')
        elif type(a.target) == ast.Subscript:
            if type(a.target.value) == ast.Name:
                names.append(a.target.value.id)
            else:
                log.info(f'transformations\tclearBlockVars\tFor target subscript not a name: {str(type(a.target.value))}, bug')
        else:
            log.info(f'transformations\tclearBlockVars\tFor target not a name: : {str(type(a.target))}, bug')
        for name in names:
            if name in liveVars:
                del liveVars[name]
    ...
    if type(a) == list:
        ...
        while i < len(a):
                ...
                if type(target) in [ast.Name, ast.Subscript, ast.Attribute]:
                    ...
                    for e in target.elts:
                        if type(e) in [ast.Name, ast.Subscript, ast.Attribute]:
                            ...
                            log.info(f'transformations\tcopyPropagation\tWeird assign type: {str(type(e))}, bug')
                    ...
                    for elt in a[i].target.elts:
                        if type(elt) == ast.Name:
                            ...
                            if type(elt.value) == ast.Name:
                                ...
                                log.info(f'transformations\tcopyPropagation\tFor target subscript not a name: '
                                         f'{str(type(elt.value))}\t{printFunction(elt.value)}, bug')

                        else:
                            log.info(f'transformations\tcopyPropagation\tFor target not a name: '
                                     f'{str(type(elt))}\t{printFunction(elt)}, bug')
                elif type(a[i].target) == ast.Subscript:
                    if type(a[i].target.value) == ast.Name:
                        names.append(a[i].target.value.id)
                    else:
                        log.info(f'transformations\tcopyPropagation\tFor target subscript not a name: '
                                 f'{str(type(a[i].target.value))}\t{printFunction(a[i].target.value)}, bug')
                else:
                    log.info(f'transformations\tcopyPropagation\tFor target not a name: '
                             f'{str(type(a[i].target))}\t{printFunction(a[i].target)}, bug')
            ...
            # These are not affected by this function
                ...
                log.info(f'transformations\tcopyPropagation\tNot implemented: {str(type(a[i]))}, bug')
            i += 1
        return a
    else:
        log.info(f'transformations\tcopyPropagation\tNot a list: {str(type(a))}, bug')
        return a
    ...
    if type(a) == list:
        ...
        while i >= 0 and len(a) > 0:
                    ...
                    for elt in a[i].target.elts:
                        if type(elt) == ast.Name:
                            ...
                            if type(elt.value) == ast.Name:
                                ...
                                log.info(f'transformations\tdeadCodeRemoval\tFor target subscript not a name: '
                                         f'{str(type(elt.value))}\t{printFunction(elt.value)}, bug')
                        else:
                            log.info(f'transformations\tdeadCodeRemoval\tFor target not a name: '
                                     f'{str(type(elt))}\t{printFunction(elt)}, bug')
                elif type(a[i].target) == ast.Subscript:
                    if type(a[i].target.value) == ast.Name:
                ...
                if type(test) == ast.NameConstant and test.value in [True, False]:
                    ...
                    for var in assignedVars:
                        ...
                        if var.id[0] == "g" and hasattr(var, "originalId"):
                            log.info(f'canonicalize\tdeadCodeRemoval\tWeird global variable: {printFunction(a[i])}, bug')
                            break
                    else:
                        if test.value == True:
                            a[i:i + 1] = a[i].body
            ...
            # Have not yet implemented these
                ...
                log.info(f'transformations\tdeadCodeRemoval\tNot implemented: {str(type(stmt))}, bug')
            i -= 1
        return a
    else:
        log.info(f'transformations\tdeadCodeRemoval\tNot a list: {str(a)}, bug')
        return a
    ...
    if type(a) == ast.BinOp:
        ...
        if type(a.op) == ast.Add:
            ...
            if l not in [int, float] or r not in [int, float]:
                return [a]
        else:  # canonicalization.Div, canonicalization.FloorDiv, canonicalization.Mod
...
def orderCommutativeOperations(a):
    ...
    if type(a) == ast.If:
        ...
        branches = [(a.test, a.body)]
        orElse = a.orelse
        while len(orElse) == 1 and type(orElse[0]) == ast.If:
            branches.append((orElse[0].test, orElse[0].body))
        ...
        if len(branches) != 1:
            ...
            if len(orElse) == 0 and isNegation(branches[-1][0], branches[-2][0]):
                ...
                starter = [ast.If(branches[-1][0], branches[-1][1], orElse)]
            # Create the new conditional tree
            for i in range(len(branches) - 2, -1, -1):
                starter = [ast.If(branches[i][0], branches[i][1], starter)]
            a = starter[0]
        return a
    ...
    if type(a) == ast.UnaryOp and type(a.op) == ast.Not:
        ...
        # not not blah == blah
            ...
            if oper.value in [True, False]:
                ...
                log.info(f'Unknown NameConstant: {str(oper.value)}, bug')
...
def turnPositive(a):
    ...
    if type(a) == ast.UnaryOp and type(a.op) == ast.USub:
        ...
        log.info(f'transformations\tturnPositive\tFailure: {str(a)}, bug')
        return a
...
def staticVars(l, vars):
    ...
    for i in range(len(l)):
        if type(l[i]) == ast.Assign:
            ...
            log.info(f'transformations\tstaticVars\tMissing type: {str(type(l[i]))}, bug')
...
def conditionalRedundancy(a):
    ...
    if type(a) == list:
        ...
        log.info(f'transformations\tconditionalRedundancy\tStrange type: {str(type(a))}, bug')
