	...
	if type(a) == ast.FunctionDef:
		for param in a.args.args:
			if type(param) == ast.arg:
				...
				log.error("transformations\tanonymizeFunctionNames\tWeird parameter type: " + str(type(param)))
	if goBackwards:
		...
		while countVariables(body[i], helper.name) > helperCount:
			...
			for j in range(len(callExpression.args)):
				...
				if countVariables(methodLines, arg.arg) == 1 and countVariables(methodLines[-1], arg.arg) == 1:
					...
					if type(arg) == ast.arg:
						...
						log.error("transformations\tmapHelper\tWeird arg: " + str(type(arg)))
			if badArgs:
				helperCount += 1
				continue
			...
			if countOccurances(helper, ast.Return) == 1:
				...
				if type(body[i]) == ast.Return and comp:
					...
					returnLine = []
					log.error("transformations\tmapHelper\tWeird removeCall: " + str(type(body[i])))
...
def simplify(a):
	...
	if not isinstance(a, ast.AST):
		...
		if len(a.targets) > 1:
			...
			for i in range(len(a.targets)-1, 0, -1):
				...
				if type(t) == ast.Name:
					...
					log.error("transformations\tsimplify\tOdd loadedTarget: " + str(type(t)))
		...
		if type(a.target) == ast.Name:
			...
			log.error("transformations\tsimplify\tOdd AugAssign target: " + str(type(a.target)))
	...
	if not isinstance(a, ast.AST):
		...
		for i in range(len(a.args.args)):
			...
			if type(arg) == ast.arg:
				...
				log.error("transformations\tpropogateMetadata\tWeird type in args: " + str(type(arg)))
			...
			if type(t) == ast.Name:
				...
				for j in range(len(t.elts)):
					...
					if type(t2) == ast.Name:
						...
						log.error("transformations\tpropogateMetadata\tOdd listTarget: " + str(type(t.elts[j])))
		return a
	elif type(a) == ast.AugAssign:
		# Turn all AugAssigns into Assigns
		...
		for key in variableMap1:
			if key in variableMap2:
				if variableMap1[key] == variableMap2[key]:
					...
					log.error("transformations\tsimplify\tvarId mismatch")
				else: # type mismatch
...
def isMutatingFunction(a):
	...
	if type(a) != ast.Call: # Can only call this on Calls!
		log.error("transformations\tisMutatingFunction\tNot a Call: " + str(type(a)))
		return True
			...
			for elt in a.target.elts:
				if type(elt) == ast.Name:
					...
					if type(elt.value) == ast.Name:
						...
						log.error("transformations\tclearBlockVars\tFor target subscript not a name: " + str(type(elt.value)))
				else:
					log.error("transformations\tclearBlockVars\tFor target not a name: " + str(type(elt)))
		elif type(a.target) == ast.Subscript:
			if type(a.target.value) == ast.Name:
				names.append(a.target.value.id)
			else:
				log.error("transformations\tclearBlockVars\tFor target subscript not a name: " + str(type(a.target.value)))
		else:
			log.error("transformations\tclearBlockVars\tFor target not a name: " + str(type(a.target)))
		for name in names:
			if name in liveVars:
				del liveVars[name]
	...
	if type(a) == list:
		...
		while i < len(a):
				...
				if type(target) in [ast.Name, ast.Subscript, ast.Attribute]:
					...
					for e in target.elts:
						if type(e) in [ast.Name, ast.Subscript, ast.Attribute]:
							...
							log.error("transformations\tcopyPropagation\tWeird assign type: " + str(type(e)))
					...
					for elt in a[i].target.elts:
						if type(elt) == ast.Name:
							...
							if type(elt.value) == ast.Name:
								...
								log.error("transformations\tcopyPropagation\tFor target subscript not a name: " + str(type(elt.value)) + "\t" + printFunction(elt.value))
						else:
							log.error("transformations\tcopyPropagation\tFor target not a name: " + str(type(elt)) + "\t" + printFunction(elt))
				elif type(a[i].target) == ast.Subscript:
					if type(a[i].target.value) == ast.Name:
						names.append(a[i].target.value.id)
					else:
						log.error("transformations\tcopyPropagation\tFor target subscript not a name: " + str(type(a[i].target.value)) + "\t" + printFunction(a[i].target.value))
				else:
					log.error("transformations\tcopyPropagation\tFor target not a name: " + str(type(a[i].target)) + "\t" + printFunction(a[i].target))
			...
			# These are not affected by this function
				...
				log.error("transformations\tcopyPropagation\tNot implemented: " + str(type(a[i])))
			i += 1
		return a
	else:
		log.error("transformations\tcopyPropagation\tNot a list: " + str(type(a)))
		return a
	...
	if type(a) == list:
		...
		while i >= 0 and len(a) > 0:
					...
					for elt in a[i].target.elts:
						if type(elt) == ast.Name:
							...
							if type(elt.value) == ast.Name:
								...
								log.error("transformations\tdeadCodeRemoval\tFor target subscript not a name: " + str(type(elt.value)) + "\t" + printFunction(elt.value))
						else:
							log.error("transformations\tdeadCodeRemoval\tFor target not a name: " + str(type(elt)) + "\t" + printFunction(elt))
				elif type(a[i].target) == ast.Subscript:
					if type(a[i].target.value) == ast.Name:
						targetNames.append(a[i].target.value.id)
					else:
						log.error("transformations\tdeadCodeRemoval\tFor target subscript not a name: " + str(type(a[i].target.value)) + "\t" + printFunction(a[i].target.value))
				else:
					log.error("transformations\tdeadCodeRemoval\tFor target not a name: " + str(type(a[i].target)) + "\t" + printFunction(a[i].target))
				...
				if type(test) == ast.NameConstant and test.value in [True, False]:
					...
					for var in assignedVars:
						...
						if var.id[0] == "g" and hasattr(var, "originalId"):
							log.error("canonicalize\tdeadCodeRemoval\tWeird global variable: " + printFunction(a[i]))
							break
					else:
						if test.value == True:
							a[i:i+1] = a[i].body
			...
			# Have not yet implemented these
				...
				log.error("transformations\tdeadCodeRemoval\tNot implemented: " + str(type(stmt)))
			i -= 1
		return a
	else:
		log.error("transformations\tdeadCodeRemoval\tNot a list: " + str(a))
		return a
	...
	if type(a) == ast.UnaryOp and type(a.op) == ast.Not:
		...
		# not not blah == blah
			...
			if oper.value in [True, False]:
				...
				log.error("Unknown NameConstant: " + str(oper.value))
...
def turnPositive(a):
	...
	if type(a) == ast.UnaryOp and type(a.op) == ast.USub:
		...
		log.error("transformations\tturnPositive\tFailure: " + str(a))
		return a
...
def staticVars(l, vars):
	...
	for i in range(len(l)):
		if type(l[i]) == ast.Assign:
			...
			log.error("transformations\tstaticVars\tMissing type: " + str(type(l[i])))
...
def conditionalRedundancy(a):
	...
	if type(a) == list:
		...
		log.error("transformations\tconditionalRedundancy\tStrange type: " + str(type(a)))
...
def collapseConditionals(a):
	...
	if type(a) == list:
		...
		log.error("transformations\tcollapseConditionals\tStrange type: " + str(type(a)))