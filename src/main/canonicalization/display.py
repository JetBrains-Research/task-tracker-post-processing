...
# TODO: add AsyncFunctionDef, AsyncFor, AsyncWith, AnnAssign, Nonlocal, Await, YieldFrom, FormattedValue, JoinedStr,
#  Starred

log = logging.getLogger(consts.LOGGER_NAME)


def printFunction(a: ast.AST, indent=0):
	s = ""
	if a is None:
		return ""
	if not isinstance(a, ast.AST):
		log.info(f'display\tprintFunction\tNot AST: {type(a)}, {str(a)} bug')
		return str(a)
	...
	# s += "'" + a.s.replace("'", "\\'").replace('"', "\\'").replace("\n","\\n") + "'"
		...
		if type(a) in [ast.Load, ast.Store, ast.Del, ast.AugLoad, ast.AugStore, ast.Param]:
			return ""
		log.info(f'display\tMissing type: {str(t)}, bug')
	return s
...
def formatContext(trace, verb):
	...
	while i < len(trace):
		if type(trace[i]) == tuple:
			if trace[i][0] == "value" and trace[i][1] == "Attribute":
				...
				log.info(f'display\tformatContext\tSkipped field: {str(trace[i])}, bug')
		i += 1
	else:
	...
	if field in traceD and typ in traceD[field]:
		...
		log.info(f'display\tformatContext\tMissing field: {str(field)}, {str(typ)}, bug')
		return ""
